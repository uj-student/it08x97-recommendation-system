import json
import requests
from urllib.request import *

URL = "https://developers.zomato.com/api/v2.1/"
KEY = "9d83b19463fa5f6cf94b1af9f5ca17fa"
ESTABLISHMENT = "establishments?city_id="
CATEGORY = "categories"
CUISINE = "cuisines?city_id="
SEARCH = "search"

city_id = 65 #id for jozi
s = "search?entity_type=city&q=Johannesburg&sort=rating&order=desc"
search = "search?entity_type={}&q={}&sort=rating&order=desc"
entity_type = "city" #search by
entity_name = "Johannesburg"

def get_resource(end_point=None, p=None):
    # response = requests.get(URL + end_point, params=p, headers={"user-key":KEY})
    response = requests.get("https://developers.zomato.com/api/v2.1/search?q=carrot", headers={"user-key":KEY})
    return response.json()


def get_establishments():
    res = get_resource(ESTABLISHMENT + str(city_id))
    if res.status_code ==200:
        data = res.json()
        return json.dumps(data, indent=2, sort_keys=True)

def get_categories():
	res = get_resource(CATEGORY)
	if res.status_code ==200:
		data = res.json()
		return json.dumps(data, indent=2, sort_keys=True)
        # for category in data['categories']:
        #     print (category['categories'])

def get_cuisines():
    res = get_resource(CUISINE + str(city_id))
    if res.status_code ==200:
        data = res.json()
        return json.dumps(data, indent=2, sort_keys=True)

def get_restaurants(entity_type, entity_name):
	res = get_resource(search.format(entity_type, entity_name))
	if res.status_code == 200:
		data = res.json()
		return( json.dumps(data, indent=2) )



url = "https://developers.zomato.com/api/v2.1/search"


param = {
    'city_id' : '65'
    # 'q' : 'mushroom'
}
def get_rest():
    res = get_resource(SEARCH, param)
    if res.status_code == 200:
        data = res.json()
        return ( json.dumps(data, indent=2, sort_keys=True))


# def write_response_to_file():
#     res = get_resource(SEARCH, param)
#     if res.status_code == 200:
#         data = res.json()
#         with open ( 'restaurant.json', 'w') as restaurant:
#             json.dumps(restaurant, data, indent=2)

# get_establishments()
# get_establishments()
# get_cuisines()
# get_restaurants(entity_type, entity_name)
var = json.dumps(get_resource(), indent=2)

print (var)
print (get_resource().header)
